...
Существует два кслассических подхода к созданию микросервисных приложений - `Microservices First <https://martinfowler.com/articles/dont-start-monolith.html>`__ и `Monolith First <https://martinfowler.com/bliki/MonolithFirst.html>`__.
И если превосходства последнего широко освещены рядом известных авторитетов в области архитектуры, такими как Sam Newman, Robert Martin, Martin Fowler и др., то об освещенности превосходств первого этого не скажешь.
...
Такой подход позволяет удешевить процесс Model Distilling, и подразумевает вывод Bounded Contexts в независимые сервисы после стабилизации контуров бизнес-моделей и границ контекстов, по мере назревания в этом реальной необходимости.

Отсюда вырисовывается первый мотив использования Monolith First - потребность в Crunching Knowledge (переработке знаний), которая не позволяет сразу очертить стабильные контуры бизнес-моделей.
...
Недостатком Monolith First является высокая чувствительность к качеству реализации (конструирования), которая требует от разработчиков высокой компетентности и дисциплины, что означает удорожание разработки, поскольку привлечь разработчиков среднего и начального уровня без ущерба изоляции компонентов не получится (creeping coupling).
А нанять одного опытного архитектора всегда выгоднее, чем укомплектовать всю команду опытными разработчиками.
Таким образом, слабость архитектуры в таком случае мы должны компенсировать сильным качеством разработки.
