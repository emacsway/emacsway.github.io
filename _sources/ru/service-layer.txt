...
Этот подход сильно напоминает мне ":ref:`Cross-Cutting Concerns <domain-model-relationship-implementing-aop-ru>`" [#fnccode]_ с тем только отличием, что "Cross-Cutting Concerns" реализует интерфейс оригинального объекта, в то время как domain facade дополняет его.
