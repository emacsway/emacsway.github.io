...
Каждый компонент спроектирован под обе реализации и может работать как в самостоятельном процессе (в виде микросервиса), так и в общем процессе приложения (в виде подключаемого компонента).
Соответственно, каждый компонент спроектирован под две реализации Сервисного Слоя, одна из которых выполнена в виде клиента для работы с Сервисом Удаленного Вызова.
Это позволяет использовать "`Evolutionary Design`_" и расщеплять монолит на микросервисы по мере назревания в этом реальной необходимости, высвобождая время разработчиков на этапе создания MVP.
